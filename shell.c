    ...
    switch(cmd->type){
        ...
        case ' ':
            ...
            if(execvp(ecmd->argv[0], ecmd->argv) == -1){
                if(strcmp(ecmd->argv[0],"exec") == 0){
                    execvp(ecmd->argv[1], &ecmd->argv[1]);
                }
                else{
                    if (execv(strcat("/usr/bin/", ecmd->argv[0]), ecmd->argv) == -1){
                        fprintf(stderr, "Could not execute %s\n",ecmd->argv[0]);
                        exit(1); // neccessary for the command to stop executing and go back to th shell if there is a problem 
                    }
                }
